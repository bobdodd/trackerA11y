    ...
    const script = `
      tell application "Safari"
        try
          ...
          -- Execute JavaScript to find element at coordinates (single line to avoid AppleScript issues)
          set jsCode to "(function(){ try{ var screenX=${x}; var screenY=${y}; var outerW=window.outerWidth; var outerH=window.outerHeight; var innerW=window.innerWidth; var innerH=window.innerHeight; var threshold=170; var widthThreshold=outerW-innerW>threshold; var heightThreshold=outerH-innerH>threshold; var devToolsOpen=false; var orientation='closed'; if(!(heightThreshold&&widthThreshold)&&(widthThreshold||heightThreshold)) { devToolsOpen=true; orientation=widthThreshold?'vertical':'horizontal'; } var visualViewportOffsetX=0; var visualViewportOffsetY=0; var hasVisualViewport=false; if(window.visualViewport) { hasVisualViewport=true; visualViewportOffsetX=window.visualViewport.offsetLeft||0; visualViewportOffsetY=window.visualViewport.offsetTop||0; } var windowX=window.screenX||0; var windowY=window.screenY||0; var chromeH=outerH-innerH; var screenLeft=window.screenLeft||0; var screenTop=window.screenTop||0; var pageX=screenX-screenLeft; var pageY=screenY-screenTop; var debugInfo='Screen:('+screenX+','+screenY+') Window:('+windowX+','+windowY+') OuterSize:('+outerW+','+outerH+') InnerSize:('+innerW+','+innerH+') Chrome:'+chromeH+' VisualViewport:'+(hasVisualViewport?'yes':'no')+' VVOffset:('+visualViewportOffsetX+','+visualViewportOffsetY+') DevTools:'+devToolsOpen+' Orientation:'+orientation+' Page:('+pageX+','+pageY+')'; if(pageX<0||pageY<0||pageX>innerW||pageY>innerH) return JSON.stringify({error:'Outside viewport',debug:debugInfo}); var el=document.elementFromPoint(pageX,pageY); if(!el) return JSON.stringify({error:'No element',debug:debugInfo}); return JSON.stringify({tagName:el.tagName,textContent:el.textContent?el.textContent.trim().substring(0,50):'',href:el.href||'',id:el.id||'',className:el.className||'',ariaLabel:el.getAttribute('aria-label')||'',role:el.getAttribute('role')||'',debug:debugInfo}); }catch(e){ return JSON.stringify({error:e.message,debug:'Exception occurred'}); } })();"
